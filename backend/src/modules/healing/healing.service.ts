...
import { HealingContent, UserHealingRecord, MeditationSession, UserFavorite } from './entities/healing-content.entity';
...
export class HealingService {
    ...
    async findOne(id: string): Promise<HealingContent> {
        const content = await this.healingContentRepository.findOne({ where: { id } });
        if (!content) {
     ...
     * 记录用户疗愈活动
     */
    async recordActivity(userId: string, contentId: string, duration?: number): Promise<UserHealingRecord> {
     ...
     * 记录冥想
     */
    async recordMeditation(userId: string, sessionData: Partial<MeditationSession>): Promise<MeditationSession> {
     ...
     * 获取冥想统计
     */
    async getMeditationStats(userId: string): Promise<{ totalMinutes: number, totalSessions: number }> {
    ...
    async toggleFavorite(userId: string, contentId: string): Promise<boolean> {
        const existing = await this.favoriteRepository.findOne({ where: { user_id: userId, content_id: contentId } });
        if (existing) {
            await this.favoriteRepository.remove(existing);
            return false;
        } else {
            const favorite = this.favoriteRepository.create({ user_id: userId, content_id: contentId });
    ...
    async isFavorited(userId: string, contentId: string): Promise<boolean> {
        const count = await this.favoriteRepository.count({ where: { user_id: userId, content_id: contentId } });
        return count > 0;
    }
