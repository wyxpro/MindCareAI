...
export class AiController {
  ...
  @ApiOperation({
    summary: '音频情绪分析',
    description: '上传音频文件，返回语音识别结果和情绪分析。支持 wav/m4a/webm/mp3 格式，最大 10MB。'
  })
  @ApiBearerAuth('JWT-auth')
  @ApiConsumes('multipart/form-data')
  ...
  async audioEmotionAnalysis(
    @UploadedFile(
      new ParseFilePipe({
        validators: [
          new MaxFileSizeValidator({ maxSize: 10 * 1024 * 1024 }), // 10MB
          new FileTypeValidator({ fileType: /(wav|m4a|webm|mp3|audio\/wav|audio\/m4a|audio\/mp4|audio\/x-m4a|audio\/webm|audio\/mpeg)$/i }),
        ],
        fileIsRequired: true,
        exceptionFactory: (error: any) => {
          throw new BadRequestException(`文件验证失败: ${error.message}`);
  ...
  async uploadFile(
    @UploadedFile(
      new ParseFilePipe({
        validators: [
          ...
          new FileExtensionValidator(['jpg', 'jpeg', 'png', 'gif', 'webp', 'mp3', 'mp4', 'wav', 'm4a', 'webm']),
        ],
        fileIsRequired: true,
      }),
    )
    ...
    const emoji = fileType === 'image' ? '📷' : fileType === 'video' ? '🎬' : fileType === 'audio' ? '🎵' : '📄';

    console.log(`\n${emoji} ========== 文件上传 ==========`);
    console.log(`📁 文件名: ${file.originalname}`);
    console.log(`📏 文件大小: ${fileSizeMB}MB (${file.size.toLocaleString()} 字节)`);
    console.log(`🎯 MIME 类型: ${file.mimetype}`);
    console.log(`💾 保存路径: ./uploads/${file.filename}`);
  ...
  @ApiOperation({
    ...
    description: '将视频上传到 StepFun 的文件存储服务，返回 stepfile:// 格式的文件 ID。' +
      '视频将被存储在 StepFun 服务器上，避免重复下载，提升 AI 视频理解速度。' +
      '支持最大 128MB 的 MP4 视频文件。'
  })
  @ApiConsumes('multipart/form-data')
  @ApiResponse({ status: 200, description: '文件上传成功，返回 stepfile:// 文件 ID' })
  ...
  async uploadToStepFun(
    ...
    try {
      ...
      console.log(`🎉 上传完成! 返回 fileId: ${fileId}`);
      console.log(`==========================================\n`);
  ...
  @ApiOperation({
    summary: '视频理解分析',
    description: '使用配置的多模态模型理解视频内容并进行分析。' +
      '需要先通过 /upload-to-stepfun 端点上传视频到 StepFun 存储，' +
      '然后传入 stepfile:// 格式的文件 ID。'
  })
  @ApiResponse({ status: 200, description: '分析成功，返回 AI 对视频内容的理解' })
  @ApiResponse({ status: 400, description: '请求参数错误' })
  @ApiResponse({ status: 429, description: '请求过于频繁，请稍后重试' })
  @ApiResponse({ status: 500, description: 'AI 服务暂时不可用' })
  async videoUnderstanding(@Body() body: {
