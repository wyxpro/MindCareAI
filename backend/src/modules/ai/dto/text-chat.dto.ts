...
import { IsArray, IsNotEmpty, IsObject, ValidateNested, IsOptional, IsString, IsIn, IsBoolean } from 'class-validator';
...
export class ChatMessageContent {
  @ApiProperty({ description: '内容类型', enum: ['text', 'image_url', 'video_url', 'input_audio'] })
...
export class ChatMessage {
  @ApiProperty({ description: '角色', enum: ['system', 'user', 'assistant', 'tool'] })
  role: 'system' | 'user' | 'assistant' | 'tool';

  @ApiProperty({ description: '消息内容', oneOf: [{ type: 'string' }, { type: 'array', items: { $ref: '#/components/schemas/ChatMessageContent' } }] })
  content: string | ChatMessageContent[];

  @ApiProperty({ description: '执行函数的ID (仅针对 tool 角色)', required: false })
  tool_call_id?: string;
}
...
export class SpeechRecognitionUploadDto {
  @ApiProperty({ description: '音频格式', enum: ['wav', 'm4a'], required: false, default: 'wav' })
  @IsOptional()
  @IsIn(['wav', 'm4a'])
  format?: 'wav' | 'm4a';
...
export class RagRetrievalDto {
  ...
  @ApiProperty({ description: '对话历史', required: false, type: [ChatMessage] })
  @IsArray()
  @IsOptional()
