...
export class EmotionDiariesController {
  ...
  async findAll(
    ...
    // 医生可以查看所有用户的日记
    const targetUserId = (user.role === 'doctor' || user.role === 'admin') && user.patientId
      ? user.patientId
      : userId;
  ...
  async findOne(
    ...
    return this.emotionDiariesService.findOne(id, userId, user.role || UserRole.USER);
  }
  ...
  async update(
    ...
    return this.emotionDiariesService.update(id, updateDto, userId, user.role || UserRole.USER);
  }
  ...
  async remove(
    ...
    return this.emotionDiariesService.remove(id, userId, user.role || UserRole.USER);
  }
}
